...
define(['text!template/plugins/form.html'], function(html) {
	...
	function loadFields() {
		...
		Heiduc.jsonrpc.fieldService.getByForm(function(r, e) {
			fields = r.list;
			showFields();
		}, formId);
	}
	...
	function showFields() {
		var h = '<table class="table table-striped"><tr><th></th><th>' + messages('title') 
	...
	function onFieldSave(closeFlag) {
		...
		if (errors.length == 0) {
			Heiduc.jsonrpc.fieldService.updateField( function(r, e) {
				if (r.result == 'success') {
					if (closeFlag) {
						$("#field-dialog").dialog("close");
					}
	...
	function fieldInfoMessage(message) {
		Heiduc.infoMessage('#field-messages', message);
	}

	function fieldErrorMessages(messages) {
		Heiduc.errorMessages('#field-messages', messages);
	}

	function fieldErrorMessage(message) {
		Heiduc.errorMessage('#field-messages', message);
	}
	...
	function onFieldEdit(fieldId) {
		clearFieldMessage();
		Heiduc.jsonrpc.fieldService.getById( function(r) {
			field = r;
			loadRegexMessage();
			fieldDialogInit();
			$("#field-dialog").dialog("open");
	...
	function onDeleteFields() {
		...
		if (confirm(messages('are_you_sure'))) {
			Heiduc.jsonrpc.fieldService.remove(function(r) {
				if (r.result == 'success') {
				    Heiduc.info(ids.length + ' ' + messages('form.success_field_delete') 
				        + '.');
				    loadFields();
				}
				else {
	...
	function loadForm() {
		Heiduc.jsonrpc.formService.getForm(function (r) {
			form = r
			loadFormData();
			if (r != null) {
				$('#title').val(r.title);
	...
	function onUpdate() {
		...
		Heiduc.jsonrpc.formService.saveForm(function (r) {
			if (r.result == 'success') {
				if (!editMode) {
					formId = r.message;
					editMode = true;
					loadData();
					Heiduc.info(messages('form.success_create'));
				}
				else {
	...
	function onFieldTitleChange() {
		...
		if (name == '') {
			$('input[name="field.name"]').val(Heiduc.urlFromTitle(title));
		}
	}

	function onFieldUp(i) {
		if (i - 1 >= 0) {
			Heiduc.jsonrpc.fieldService.moveUp(function(r) {}, formId, fields[i].id);
	        fields[i].index--;
	        fields[i - 1].index++;
			swapFields(i, i - 1);
			showFields();
	...
	function onFieldDown(i) {
		if (i + 1 < fields.length) {
			Heiduc.jsonrpc.fieldService.moveDown(function(r) {}, formId, fields[i].id);
	        fields[i + 1].index--;
	        fields[i].index++;
			swapFields(i, i + 1);
			showFields();
	...
	function loadFormData() {
		if (form == null) {
			...
			Heiduc.jsonrpc.formService.getFormData(function(r) {
				formData = r.list;
				showFormData();
			}, form.id);
		}		
	...
	function showFormData() {
		var h = '<table class="table table-striped"><tr><th></th><th>' + messages('ip_address') 
	...
	function onDeleteData() {
		...
		if (confirm(messages('are_you_sure'))) {
			Heiduc.jsonrpc.formService.removeData(function(r) {
				if (r.result == 'success') {
					Heiduc.info(ids.length + ' ' + messages('form.success_records_delete') 
					    + '.');
					loadData();
				}
				else {
	...
	function onShowFormData(i) {
		formDataIndex = i;
		var data = formData[i];
		var h = '<table class="table table-striped"><tr><th>' + messages('name') 
	...
	function onFormDataSend() {
		if (confirm(messages('are_you_sure'))) {
			Heiduc.jsonrpc.formService.sendFormLetter(function(r) {
				Heiduc.showServiceMessages(r);
