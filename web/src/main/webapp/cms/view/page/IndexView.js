...
function(indexHtml, ctx, version, breadcrumbs) {
	...
	function getPublishDatetime() {
		return Heiduc.strip($('#publishDate').val()) 
			+ ' ' + Heiduc.strip($('#publishTime').val()) + ':00';	
	}

	function getEndPublishDatetime() {
		if ($('#endPublishDate').val()) {
			return Heiduc.strip($('#endPublishDate').val()) 
				+ ' ' + Heiduc.strip($('#endPublishTime').val()) + ':00';
		}
		return '';
	}
	...
	function onTitleChange() {
		...
		var title = $("#title").val();
		if (url == '') {
			$("#friendlyUrl").val(Heiduc.urlFromTitle(title));
		}
	}
	...
	function loadContents() {
		if (ctx.editMode) {
			...
			if (allowedLangs[Heiduc.ENGLISH_CODE] != undefined) {
	...
	function onTagSelect(id) {
		Heiduc.jsonrpc.tagService.addTag(function(r){
			Heiduc.showServiceMessages(r);
		    //$('#tag-dialog').dialog('close');
		    $('#tag-dialog').modal({show:false});
			callLoadTags();
		}, ctx.page.friendlyURL, id);
	}

	function callLoadTags() {
		Heiduc.jsonrpc.pageService.getPageTags(function(r) {
