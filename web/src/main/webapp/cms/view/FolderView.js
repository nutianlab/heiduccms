...
function(html) {
	...
	function postRender() {
	    ...
	    editMode = folderId != '';
	    //var tab = $("#tabs").tabs();
	    //Heiduc.selectTabFromQueryParam(tab);
	...
	function loadData() {
		Heiduc.jsonrpc.folderService.getFolderRequest(function (r) {
			folderRequest = r;
		    loadFolder();
		    loadGroups();
		    showBreadcrumbs();
	...
	function callLoadFiles() {
		Heiduc.jsonrpc.fileService.getByFolder(function (r) {
	...
	function onDeleteFiles() {
	    var ids = Heiduc.javaList(getCheckedFilesIds());
	    if (ids.list.length == 0) {
	    	Heiduc.info(messages('nothing_selected'));
	        return;
	    }
	    if (confirm(messages('are_you_sure'))) {
	    	Heiduc.jsonrpc.fileService.deleteFiles(function (r) {
	            if (r.result == "success") {
	                callLoadFiles();
	                Heiduc.info(messages('folder.files_success_delete'));
	            }
	            else {
	            	Heiduc.error(messages('folder.error_deleting_files') + ' ' 
	            			+ r.messsage);
	            }
	        }, ids);
	    }
	...
	function callLoadChildren() {
		Heiduc.jsonrpc.folderService.getByParent(function(r) {
	...
	function onUpdate() {
	    ...
	    Heiduc.jsonrpc.folderService.saveFolder(function (r) {
	        if (r.result == 'success') {
	        	Heiduc.info(messages('folder.success_save'));
	            if (folderId == '') {
	            	folderId = r.message;
	            	editMode = true;
	            	$('#file-upload input[name="folderId"]').val(folderId);
	...
	function onExport() {
	    ...
	    Heiduc.jsonrpc.configService.startExportFolderTask(function(r) {
	    	if (r.result == 'success') {
	...
	function onDelete() {
	    var ids = Heiduc.javaList(getParentFolderIds());
	    if (ids.list.length == 0) {
	    	Heiduc.info(messages('nothing_selected'));
	        return;
	    }
	    if (confirm(messages('are_you_sure'))) {
	    	Heiduc.jsonrpc.folderService.deleteFolder(function (r) {
	            if (r.result == "success") {
	                callLoadChildren();
	                Heiduc.info(messages('folder.success_delete'));
	            }
	            else {
	            	Heiduc.error(messages('folder.error_deleting') + ' ' 
	            			+ r.messsage);
	            }
	        }, ids);
	    }
	...
	function loadPermissions(r) {
		...
		var h = '<table class="table table-striped"><tr><th></th><th>' + messages('group') 
	...
	function callLoadPermissions() {
		Heiduc.jsonrpc.folderPermissionService.selectByFolder(function (r) {
			loadPermissions(r);
		}, folderId);
	}
	...
	function onPermissionSave() {
		...
		Heiduc.jsonrpc.folderPermissionService.save(function(r) {
			Heiduc.showServiceMessages(r);
	...
	function onDeletePermission() {
		...
		if (confirm(messages('are_you_sure'))) {
			Heiduc.jsonrpc.folderPermissionService.remove(function(r) {
				Heiduc.showServiceMessages(r);
				callLoadPermissions();
			}, Heiduc.javaList(ids));
		}
