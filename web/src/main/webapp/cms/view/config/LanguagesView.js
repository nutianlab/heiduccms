...
function(tmpl) {
	...
	function onAddLanguage() {
	    language = null;
		$('#language-dialog').modal({show:true});//.dialog('open');
	...
	function onRemoveLanguage() {
	    ...
	    if (confirm(messages('are_you_sure'))) {
	    	Heiduc.jsonrpc.languageService.remove(function (r) {
	...
	function languageInfo(msg) {
		Heiduc.infoMessage('#language-dialog .messages', msg);
	}

	function languageError(msg) {
		Heiduc.errorMessage('#language-dialog .messages', msg);
	}

	function languageErrors(errors) {
		Heiduc.errorMessages('#language-dialog .messages', errors);
	}
	...
	function onLanguageSave() {
	    ...
	    if (errors.length == 0) {
	    	Heiduc.jsonrpc.languageService.save(function (r) {
	            if (r.result == 'success') {
	...
	function onLanguageEdit(id) {
		Heiduc.jsonrpc.languageService.getById(function (r) {
	        language = r;
